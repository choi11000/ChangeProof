...
ChangeProof는 소프트웨어 엔지니어링의 변경 안전성을 증명하기 위해 **결정론적 팩트 분석**, **증거 기반 AI 가설 수립**, **격리된 PostgreSQL 샌드박스 관측**, **수학적 판정 불변식 검증**의 4단계 신뢰 계층을 결합한 아키텍처를 채택하고 있습니다.
...
flowchart TD
    ...
    subgraph EXECUTION["3. 샌드박스 관측 계층 (Real DB Observation)"]
        ...
        EXEC["Experiment Executor (asyncpg)"]
...
| **4. VERDICT & PROOF (판정/증명)** | Deterministic Verifier | **수학적 불변식 검증**. 관측된 SQLSTATE와 실험 계약 다이제스트의 일치 여부를 대조해 판정. 동일 실험 재실행으로 해결 증명. | `PROVEN_FAIL`, `PROVEN_FIXED` |

---

## 3. 핵심 안전 및 보안 아키텍처 (Safety & Security Architecture)

### A. 프롬프트 인젝션 차단 (Prompt Injection Isolation)
...
* LLM이 임의의 SQL 문장이나 셸 스크립트를 생성하여 서버에서 실행할 수 있는 통로는 시스템 상에 원천 차단되어 있습니다.
...
* **리소스 가드레일**:
  * 문장별 실행 타임아웃: `statement_timeout = 5000` (5초)
  * 락 획득 타임아웃: `lock_timeout = 2000` (2초)
  * 실험 완료/실패 여부와 관계없이 Python `finally` 블록에서 `DROP SCHEMA ... CASCADE`를 통해 100% 자원을 정리합니다.

### C. 계약 다이제스트 기반 수정 증명 (Contract Digest Proof)
