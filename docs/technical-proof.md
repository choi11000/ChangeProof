...
| **정적 분석 및 린트** | Ruff linter | **0 경고, 0 오류 (Clean)** | PEP 8 및 Python 3.13 런타임 기준 준수 |
...
* **API 백엔드**: [https://changeproof-api-production.up.railway.app](https://changeproof-api-production.up.railway.app) (FastAPI, Python 3.13)
...
5. **Deterministic Remediation**: 동일 실험 계약(`contract_...`) 하에서 복구 마이그레이션 적용 후 기존 실패가 사라진 **`PROVEN_PASS`** 관측 $\rightarrow$ 해당 before/after 실험 쌍에 대한 **`PROVEN_FIXED`** 판정.
...
* **엄격한 리소스 제한**: 샌드박스 동시 실행 수 제한(최대 2개), 문장별 타임아웃(10초), 락 타임아웃(5초), 실행 후 `finally` 블록의 스키마 삭제(`DROP SCHEMA ... CASCADE`)와 cleanup 결과 검증을 수행합니다.
...
`PROVEN_PASS`는 해당 controlled experiment에서 예상 실패가 관찰되지 않았다는 뜻이며, PR 전체 또는 production system의 안전성을 의미하지 않습니다.
