...
* **대안 후보 1 (51자)**:
  > **PR의 데이터베이스 위험을 점수로 추측하지 않고 실제 PostgreSQL에서 재현해 증명합니다.**
* **대안 후보 2 (34자)**:
  > **코드 리뷰의 추측을 실제 데이터베이스 증거로 증명합니다.**

### B. 매우 짧은 설명 (100–150자)
* **글자 수**: 138자 (공백 포함)
> ChangeProof는 GitHub PR의 데이터베이스 변경사항과 미변경 소스코드 간의 숨은 의존성을 추적하고, 격리된 PostgreSQL 샌드박스에서 실제 런타임 장애(SQLSTATE 42703)를 재현하여 배포 전 안전성을 증명하는 실행형 검증 에이전트입니다.

### C. 짧은 설명 (250–400자)
* **글자 수**: 352자 (공백 포함)
> 현대의 코드 리뷰와 AI 리뷰어는 "무엇이 바뀌었는가"와 "위험해 보이는가"는 답하지만, "이 변경이 실제 의존 코드를 부러뜨리는가"는 증명하지 못합니다. 스키마 변경은 diff에 없는 미변경 코드와 결합할 때 치명적인 장애를 만듭니다. ChangeProof는 PR의 SQL 마이그레이션과 앱 소스코드를 분석해 의존성 증거를 도출하고, AI로 구체적 장애 가설을 세운 뒤, 격리된 PostgreSQL에서 실제 실험을 실행해 실패를 증명(PROVEN_FAIL)합니다. 나아가 호환성 복구를 적용해 동일한 실험이 통과(PROVEN_PASS)하는 완전한 수정 증명(PROVEN_FIXED)을 제공합니다.
...
> 3. **격리된 PostgreSQL 실험 실행**: 제안된 실험 계획을 결정론적 컴파일러로 검증한 뒤, 수초 내에 프로비저닝되는 격리된 PostgreSQL 샌드박스에서 기준 스키마 적용, 시드 데이터 적재, PR 마이그레이션 실행, 참조 쿼리 관측을 순차 수행합니다. 실제 데이터베이스 엔진이 반환한 `SQLSTATE 42703` (undefined_column) 오류를 포착하여 명확한 증거 기반 판정(`PROVEN_FAIL`)을 내립니다.
> 4. **호환성 복구 및 동일 실험 증명**: 구버전 호환성을 유지하는 복구 마이그레이션을 적용한 후, 완전히 동일한 실험 계약(Contract Digest)을 재실행하여 통과(`PROVEN_PASS`)를 확인하고, 최종적으로 결함이 완전히 해결되었음(`PROVEN_FIXED`)을 수학적 불변식으로 증명합니다.
...
>
> 1. **SQL AST & 의존성 분석 엔진 (Python 3.12, sqlglot)**: 정규표현식이 아닌 SQL AST 파서를 통해 테이블/컬럼 변경을 구조화하고, PR 기준 커밋(`head_sha`)의 전체 소스 트리를 탐색하여 직접 참조와 테이블 컨텍스트 매칭을 정확히 분류합니다.
...
> 3. **격리된 PostgreSQL 샌드박스 실행기 (asyncpg)**: 매 실험마다 고유한 임시 네임스페이스(`cp_run_<hex12>`)를 생성하고, 문장별 타임아웃(5초) 및 락 타임아웃(2초)을 적용하여 안전하게 6단계 실험을 수행합니다. 실패 시 PostgreSQL 표준 오류 코드(`SQLSTATE 42703`)를 포착하며, `finally` 블록에서 100% 자원을 정리합니다.
...
> ChangeProof는 사람이 눈으로 찾기 어려운 숨은 코드 연결 고리를 찾아내고, 진짜 데이터베이스 샌드박스에서 변경사항을 미리 실행해 보며 진짜로 에러가 나는지 100% 증명해 줍니다. 추측이 아닌 진짜 증거를 확인하고 배포하세요!
...
2. **AI 코딩 어시스턴스 (Codex / Antigravity / Claude 3.7 Sonnet)**:
...
* **시스템/인간의 통제**: 
  ...
  * 실제 장애 증명은 순수 격리 PostgreSQL 데이터베이스의 런타임 에러(`SQLSTATE`) 관측에 의해서만 성립됩니다.
...
| **AI 활용 적절성**<br>(Appropriateness of AI Usage) | 예선 80% / 본선 포함 | - **결정론과 AI의 엄격한 분리**: 팩트 수집과 판정에는 결정론 엔진을, 다차원 관계 해석과 가설 수립에는 LLM을 배치하는 최적의 역할 분담.<br>- **보안 및 환각 방지**: 소스코드 프롬프트 인젝션 차단, Structured Outputs 강제, AI 출력은 항상 `UNVERIFIED`로 격리.<br>- **신뢰할 수 있는 결과**: AI가 "괜찮다/위험하다"를 말하지 않고, AI의 가설을 실제 DB 엔진이 검증하도록 강제함으로써 LLM의 본질적 신뢰도 문제 완벽 극복. |
